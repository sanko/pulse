## Build and Test Summary
| OS | Arch | Compiler | Result |
|:---|:---|:---|:---|
| **dragonflybsd** | `x86_64` | `gcc` | ❌ failure |
| **freebsd** | `aarch64` | `clang` | ❌ failure |
| **freebsd** | `aarch64` | `gcc` | ❌ failure |
| **freebsd** | `x86_64` | `clang` | ❌ failure |
| **freebsd** | `x86_64` | `gcc` | ❌ failure |
| **macos** | `aarch64` | `clang` | ❌ failure |
| **macos** | `aarch64` | `gcc` | ❌ failure |
| **macos** | `x86_64` | `clang` | ❌ failure |
| **macos** | `x86_64` | `gcc` | ❌ failure |
| **netbsd** | `aarch64` | `gcc` | ❌ failure |
| **netbsd** | `x86_64` | `gcc` | ❌ failure |
| **omnios** | `x86_64` | `gcc` | ❌ failure |
| **openbsd** | `aarch64` | `clang` | ❌ failure |
| **openbsd** | `aarch64` | `egcc` | ❌ failure |
| **openbsd** | `x86_64` | `clang` | ❌ failure |
| **openbsd** | `x86_64` | `egcc` | ❌ failure |
| **solaris** | `x86_64` | `gcc` | ❌ failure |
| **ubuntu** | `aarch64` | `clang` | ❌ cancelled |
| **ubuntu** | `aarch64` | `gcc` | ❌ cancelled |
| **ubuntu** | `x86_64` | `clang` | ✅ success |
| **ubuntu** | `x86_64` | `gcc` | ✅ success |
| **windows** | `aarch64` | `clang` | ❌ failure |
| **windows** | `aarch64` | `gcc` | ❌ failure |
| **windows** | `aarch64` | `msvc` | ❌ failure |
| **windows** | `x86_64` | `clang` | ❌ failure |
| **windows** | `x86_64` | `gcc` | ❌ failure |
| **windows** | `x86_64` | `msvc` | ✅ success |
