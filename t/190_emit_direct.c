...
#elif defined(PULSE_ARCH_ARM64)
static const emit_register_t ABI_GPRS[8] = {
    EMIT_REG_X0, EMIT_REG_X1, EMIT_REG_X2, EMIT_REG_X3, EMIT_REG_X4, EMIT_REG_X5, EMIT_REG_X6, EMIT_REG_X7};
#define ABI_GPR_COUNT 8
#endif
...
static void pulse_emit_call(
    ...
    for (size_t i = 0; i < num_args && i < (size_t)ABI_GPR_COUNT; i++) {
        emit_register_t phys = pulse_vreg_alloc(ctx, alloc, arg_vregs[i], P_TYPE_INT);
        if (phys != ABI_GPRS[i])
            emit_math_mov_reg(ctx, ABI_GPRS[i], phys);
    }
        ...
        case P_OP_LOAD_FLOAT:
            {
                ...
                emit_math_push(ctx, tmp);
                emit_register_t d = pulse_vreg_alloc(ctx, alloc, in->dest_vreg, P_TYPE_FLOAT);
                emit_math_movsd_reg(ctx, d, 0);  // Not great, but hardcoded XMM0
                emit_math_pop(ctx, tmp);
                pulse_vreg_free(alloc, MAX_VREGS - 1);
#endif
                break;
...
TEST {
    ...
    subtest("MOV instruction") {
        ...
        if (exec_mem) {
            ...
            ok(((emit_test_fn_0)exec_mem)() == 42, "identity() == 42");
            free_executable(exec_mem, 6);
        }
        else
            fail("exec fail");
#else
        skip(1, "X64 hardcoded only");
#endif
    }
    ...
    subtest("JMP and RELOCATION") {
        ...
        void * mem;
        if (execute_jit_code(code, sz, &mem)) {
            ok(((emit_test_fn_0)mem)() == 42, "direct target ok");
            ok(((emit_test_fn_0)((uint8_t *)mem + caller_off))() == 42, "relocated call ok");
            free_executable(mem, sz);
        }
        emit_destroy(ctx);
    }
    ...
    subtest("Feature 9: Fibers") {
        ...
        fib->stack_mem = malloc(64 * 1024);
        fib->rsp = (uintptr_t)fib->stack_mem + (64 * 1024) - 64;
    ...
    subtest("Variadic function pointer test") {
        ...
        /* 1. Prologue to save return address */
        (void)emit_math_prologue(ctx);

        /* 2. Load the function pointer from globals into a register */
#if defined(PULSE_ARCH_X64)
        emit_register_t fn_reg = EMIT_REG_RAX;
#else
        emit_register_t fn_reg = EMIT_REG_X9;  // Use a scratch register
#endif
        (void)emit_math_load_sym(ctx, fn_reg, "variadic_fn_ptr");

        /* 3. Null check: if (fn_ptr == 0) jump to skip */
        (void)emit_math_test(ctx, fn_reg, fn_reg);
        (void)emit_math_jmp_cc(ctx, EMIT_CC_E, "variadic_skip");

        /* 4. Prepare variadic call.
           On x64 SysV, AL must be 0 (number of floating point regs).
           On ARM64 standard, X0-X7 are used.
           On ARM64 Apple, we'd need to push to stack (but this test has 0 args). */
#if defined(PULSE_ARCH_X64)
        (void)emit_math_mov_imm(ctx, EMIT_REG_RAX, 0);
#endif

        /* 5. Perform the call */
        (void)emit_math_call_reg(ctx, fn_reg);

        /* 6. Store result (RAX/X0) into global */
        (void)emit_math_store_sym(ctx, "variadic_result", TEST_REG_RET);

        (void)emit_emit_label(ctx, "variadic_skip");

        /* 7. Epilogue and Return */
        (void)emit_math_epilogue(ctx);
        ...
        void * exec_mem = NULL;
        if (execute_jit_code(code, code_size, &exec_mem)) {
            // pulse_debug_hex_dump("Variadic Call", exec_mem, code_size);
    ...
    subtest("Namespaces & Operators") {
        plan(2);
        emit_context_t * ctx = create_test_context();
        emit_add_section(ctx, ".data", EMIT_SECTION_FL | EMIT_SECTION_FLAG_WRITE);
        emit_begin_section(ctx, ".data");
        emit_define_symbol(ctx, "argX", EMIT_VISIBILITY_DEFAULT, false);
        emit_emit_u64(ctx, 10);
        emit_define_symbol(ctx, "argY", EMIT_VISIBILITY_DEFAULT, false);
