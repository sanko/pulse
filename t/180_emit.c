/**
 * @file 180_emit.c
 * @brief Unit test for the emit API (JIT compiler and Executable builder suite).
 * @ingroup test_suite
 */
...
static int run_disk_executable(const char * path) {
    char cmd[256];
#ifdef _WIN32
    snprintf(cmd, sizeof(cmd), "%s", path);
#else
    snprintf(cmd, sizeof(cmd), "chmod +x %s && ./%s", path, path);
#endif
    int ret = system(cmd);
#ifndef _WIN32
    if (WIFEXITED(ret))
...
static int setup_test_section(emit_context_t * ctx) {
    pulse_status status =
        emit_add_section(ctx, EMIT_TEST_SECTION, EMIT_SECTION_FLAG_ALLOC | EMIT_SECTION_FLAG_EXECUTE);
    if (status != PULSE_SUCCESS)
        return 0;
...
TEST {
...
#else
        skip(1, "No hardcoded MOV for this arch");
        return;
#endif
    ...
    subtest("Pointer handling") {
        ...
        if (execute_jit_code(code, code_size, &exec_mem)) {
            ...
            *data_value = 123;
            ok((uint64_t)load_fn() == 123, "load_ptr: loaded value is 123");
            *data_value = 0xDEADBEEF;
            ok((uint64_t)load_fn() == 0xDEADBEEF, "load_ptr: loaded 0xDEADBEEF");
            free_executable(exec_mem, code_size);
        }
        emit_destroy(ctx);
    }
    ...
    subtest("Large struct") {
        ...
        if (execute_jit_code(code, code_size, &exec_mem)) {
            ...
            *large_ptr = (uint64_t)field0;
            emit_test_fn_0 sum_fn = (emit_test_fn_0)((uint8_t *)exec_mem + data_section_size);
            *field0 = 100;
            *((volatile uint64_t *)((uint8_t *)exec_mem + 16)) = 200;
            (void)sum_fn();
    ...
    subtest("Mixed types in global struct") {
        ...
        if (execute_jit_code(code, code_size, &exec_mem)) {
            ...
            *(volatile uint64_t *)exec_mem = (uintptr_t)f0;
    ...
    subtest("Multiple functions modifying same global") {
        ...
        if (execute_jit_code(code, code_size, &exec_mem)) {
            ...
            emit_test_fn_0 square_fn =
                (emit_test_fn_0)((uint8_t *)exec_mem + data_section_size + add_ten_offset);
            *x = 5;
            (void)double_fn();
    ...
    subtest("Relocations") {
        ...
        (void)emit_math_mov_imm(ctx, TEST_REG_RET, 0); // Failure path
        (void)emit_math_ret(ctx);

        (void)emit_emit_label(ctx, "target");
        (void)emit_math_mov_imm(ctx, TEST_REG_RET, 1); // Success path
        (void)emit_math_ret(ctx);
        ...
        if (execute_jit_code(code, code_size, &exec_mem)) {
            emit_test_fn_0 fn = (emit_test_fn_0)exec_mem;
            uint64_t result = fn();
            ok(result == 1, "relocation resolved correctly (jump followed)");
            free_executable(exec_mem, code_size);
        }
        emit_destroy(ctx);
    }

    subtest("execute PE executable") {
#ifdef _WIN32
        plan(2);
        emit_context_t * ctx = create_test_context();
        emit_write_pe_exec(ctx, "pulse_emit_test_exec.exe", 42);
        emit_destroy(ctx);
        int ret = run_disk_executable("pulse_emit_test_exec.exe");
        ok(ret == 42, "Disk executable returned 42");
        remove("pulse_emit_test_exec.exe");
        ok(1, "Cleaned up");
#else
        plan(1);
        skip(1, "Not relevant on Linux");
#endif
    }

    subtest("write and execute ELF") {
#ifdef __linux__
        plan(2);
        emit_context_t * ctx = create_test_context();
        ...
        free(data);
        emit_destroy(ctx);
        int ret = run_disk_executable("pulse_emit_test_exec.elf");
        ok(ret == 42, "Disk ELF executable returned 42 natively");
        remove("pulse_emit_test_exec.elf");
        ok(1, "Cleaned up");
#else
        plan(1);
        skip(1, "Linux only test");
#endif
    }
}
